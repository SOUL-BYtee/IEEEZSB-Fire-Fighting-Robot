...
byte Speed = 160; // Write The Duty Cycle 0 to 255 Enable for Motor Speed
int s1, s2, s3;
...
void setup(){ // put your setup code here, to run once
  ...
  for (byte angle = 90; angle <= 140; angle += 5)
    servoPulse(servo, angle);
  for (byte angle = 140; angle >= 40; angle -= 5)
    servoPulse(servo, angle);
  for (byte angle = 40; angle <= 95; angle += 5) 
    servoPulse(servo, angle);
